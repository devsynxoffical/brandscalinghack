export const caseStudies = [
  {
    id: 'luxe-eyewear',
    brand: 'Sleep & Wellness DTC',
    niche: 'Health & Wellness',
    scale: '€0 → €118,000 / month',
    roas: '4.90x ROAS',
    adSpend: '€24,000 / mo',
    timeframe: 'Month 1',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg',
    headline: 'Scaling wellness brand to €118,000 in Month 1 with direct-response hook architecture',
    summary: 'Built dedicated UGC hook testing framework, custom high-speed Shopify landing page with 4.2% CVR, and scaled Meta Advantage+ campaigns.',
    metrics: [
      { label: 'Monthly Revenue', value: '€118,000' },
      { label: 'Blended ROAS', value: '4.90x' },
      { label: 'Store CVR', value: '4.2%' },
      { label: 'AOV Increase', value: '+38%' }
    ],
    growthPoints: [
      'Engineered 45+ variation UGC angle matrix addressing root problems',
      'Implemented 1-click bundle upsells boosting AOV',
      'Scaled daily spend from $300/day to $5,000/day with consistent CPA'
    ]
  },
  {
    id: 'glow-skincare',
    brand: 'DTC Fitness & Recovery',
    niche: 'Fitness & Recovery',
    scale: '€0 → €123,000 / 20 Days',
    roas: '5.20x ROAS',
    adSpend: '€23,500 / 20d',
    timeframe: '20 Days',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg',
    headline: 'Scaling fitness recovery brand to €123,000 in 20 days with Advantage+ scale systems',
    summary: 'Restructured media buying funnel into a high-intent Meta retargeting flywheel with problem/solution video creatives.',
    metrics: [
      { label: 'Revenue in 20 Days', value: '€123,000' },
      { label: 'Blended ROAS', value: '5.20x' },
      { label: 'CAC Reduction', value: '-42%' },
      { label: 'Repeat Customer Rate', value: '29%' }
    ],
    growthPoints: [
      'Eliminated audience overlap fatigue across ad sets',
      'Created split-screen problem/solution reels generating millions of views',
      'Built automated Klaviyo flow recovering 18% of abandoned checkouts'
    ]
  },
  {
    id: 'zen-sleep-tech',
    brand: 'DTC Brand Portfolio',
    niche: 'Multi-Brand Scaling',
    scale: '$50k → $3.5M Portfolio',
    roas: '4.80x ROAS',
    adSpend: '$720,000 / yr',
    timeframe: 'Scaled Track Record',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg',
    headline: 'Scaling 8-figure eCommerce portfolio to $100k/mo net profit with proprietary creative engine',
    summary: 'Combined unboxing reels, founder direct-response storytelling, and conversion rate optimization to turn winner SKUs into category leaders.',
    metrics: [
      { label: 'Portfolio Value', value: '$3,500,000' },
      { label: 'Blended ROAS', value: '4.80x' },
      { label: 'Net Profit Target', value: '$100k/mo' },
      { label: 'Managed Ad Spend', value: '$50M+' }
    ],
    growthPoints: [
      'Generated 80+ creator assets per month on a fixed delivery cadence',
      'Optimized mobile checkout funnel decreasing bounce rate by 34%',
      'Scaled international localized campaigns across US, UK, CA, and AU'
    ]
  }
];

export const liveSessionsData = [
  {
    id: 'session-1',
    title: 'How We Scaled From $0 to $1M/Month: The Exact Media Buying Blueprint',
    host: 'Gaurav Kapoor • 8-Figure Masterclass',
    duration: '48:20',
    date: 'Sept 14',
    thumbnail: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg',
    videoUrl: 'https://www.instagram.com/reel/Db3hW_mupo1/',
    description: 'Deep dive into account structure, creative testing cadences, budgeting rules, and when to scale bids vs duplicate winners.'
  },
  {
    id: 'session-2',
    title: 'The 2026 Creative Engine: Finding Winning Hooks That Cut CAC in Half',
    host: 'Gaurav Kapoor • Live Workshop',
    duration: '35:15',
    date: 'Sept 08',
    thumbnail: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg',
    videoUrl: 'https://www.instagram.com/reel/DbBMshACSuX/',
    description: 'How to script, shoot, and iterate high-converting UGC concepts without burning budget on random unvetted creators.'
  },
  {
    id: 'session-3',
    title: 'Shopify CRO Masterclass: Turning Cold Traffic into High-AOV Buyers',
    host: 'Gaurav Kapoor & Growth Team',
    duration: '52:40',
    date: 'Aug 29',
    thumbnail: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg',
    videoUrl: 'https://www.instagram.com/reel/C9CPs88t1qa/',
    description: 'Live website teardowns analyzing product pages, trust badges, checkout velocity, and offer architecture that converts at 4%+.'
  },
  {
    id: 'session-4',
    title: 'From Dropshipping to 9-Figure Real Brand: Transition Blueprint',
    host: 'Gaurav Kapoor • Keynote',
    duration: '1:12:00',
    date: 'Aug 18',
    thumbnail: '/assets/instagram_thumbs/insta_4_DbCVqzFhiLU.jpg',
    videoUrl: 'https://www.instagram.com/reel/DbCVqzFhiLU/',
    description: 'The step-by-step framework to transition from supplier arbitrage to custom tooling, proprietary branding, and private equity exits.'
  }
];

export const viralCreativesData = [
  {
    id: 'c1',
    title: 'Now I Wake Up Calm, Focused & Rested',
    hookType: 'Problem-Solution Relief',
    format: 'Instagram Reel (9:16)',
    views: '4.8M Views',
    roas: '4.9x',
    angle: 'Contrarian Value Comparison',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg',
    url: 'https://www.instagram.com/reel/Db3hW_mupo1/',
    scriptSnippet: 'Stop paying for fragmented routines when this single daily formula replaces 80% of your morning stress...'
  },
  {
    id: 'c2',
    title: 'Tension, Stress & Recovery Breakdown',
    hookType: 'Pattern Interrupt',
    format: 'Instagram Reel (9:16)',
    views: '3.2M Views',
    roas: '5.2x',
    angle: 'Curiosity & Before/After Contrast',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg',
    url: 'https://www.instagram.com/reel/DbBMshACSuX/',
    scriptSnippet: 'If your muscle recovery looks like this, your routine is broken. Watch what happens after day 3...'
  },
  {
    id: 'c3',
    title: 'Scale to $100k/mo Net Profit',
    hookType: '8-Figure Authority Breakdown',
    format: 'Instagram Reel (9:16)',
    views: '6.1M Views',
    roas: '4.8x',
    angle: 'Insider Secret Unveiling',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg',
    url: 'https://www.instagram.com/reel/C9CPs88t1qa/',
    scriptSnippet: 'Most eCommerce founders think more ad spend fixes low sales. Here is the bottleneck destroying your margins...'
  }
];

export const liveResultsProof = [
  {
    id: 'reel-1',
    shortcode: 'DAK4iLGSGI_',
    url: 'https://www.instagram.com/reel/DAK4iLGSGI_/',
    brand: 'Meta Ads Scaling Engine',
    hook: 'Scaling Advantage+ campaigns to 7-figures using broad targeting, weekly creative testing cadences, and cash flow architecture.',
    revenue: '$1,400,000 Scaled',
    roas: '5.2x ROAS',
    badge: 'Advantage+ Architecture',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg'
  },
  {
    id: 'reel-2',
    shortcode: 'C9RU-C9yhfU',
    url: 'https://www.instagram.com/reel/C9RU-C9yhfU/',
    brand: 'Creative Engine UGC Studio',
    hook: 'Why the first 3 seconds dictate 80% of ad spend profitability. 45+ variation direct-response creative matrix.',
    revenue: '€180,000 in Month 1',
    roas: '4.8x ROAS',
    badge: 'Viral Hook Matrix',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg'
  },
  {
    id: 'reel-3',
    shortcode: 'C9CPs88t1qa',
    url: 'https://www.instagram.com/reel/C9CPs88t1qa/',
    brand: 'DTC Brand Portfolio',
    hook: 'Profit Accelerate: Scale to $100k/mo Net Profit with $30k-$40k net margin.',
    revenue: '$3,500,000 Portfolio',
    roas: '4.8x ROAS',
    badge: '$3.5M Case Study',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg'
  },
  {
    id: 'reel-4',
    shortcode: 'DcO79UbMIFx',
    url: 'https://www.instagram.com/reel/DcO79UbMIFx/',
    brand: 'Zero to Scale',
    hook: 'If I had to start from ZERO today and build compounding monthly revenue, these are the 5 exact systems I bet on.',
    revenue: '$10,000 / mo Blueprint',
    roas: '6.1x ROAS',
    badge: 'Zero to $10k Blueprint',
    category: 'Zero to Scale',
    image: '/assets/instagram_thumbs/insta_4_DbCVqzFhiLU.jpg'
  },
  {
    id: 'reel-5',
    shortcode: 'DcLYkuoBQbo',
    url: 'https://www.instagram.com/reel/DcLYkuoBQbo/',
    brand: 'Offer Architecture & CRO',
    hook: 'How to double average order value with 1-click Shopify bundle upsells without increasing acquisition spend.',
    revenue: '+38% AOV Lift',
    roas: '4.4x ROAS',
    badge: 'Shopify 1-Click Bundles',
    category: 'CRO & Funnels',
    image: '/assets/instagram_thumbs/insta_5_DcO79UbMIFx.jpg'
  },
  {
    id: 'reel-6',
    shortcode: 'Db3hW_mupo1',
    url: 'https://www.instagram.com/reel/Db3hW_mupo1/',
    brand: 'Sleep & Wellness DTC',
    hook: 'Now I wake up calm, focused, and actually rested',
    revenue: '€118,000 in Month 1',
    roas: '4.9x ROAS',
    badge: 'Verified First Month',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_6_DZcJNI0EhWY.jpg'
  },
  {
    id: 'reel-7',
    shortcode: 'DbCVqzFhiLU',
    url: 'https://www.instagram.com/reel/DbCVqzFhiLU/',
    brand: 'Nutraceuticals Scale',
    hook: 'Deficiencies in: iron, zinc, b-vitamins... clinical angles scaled on Meta Ads.',
    revenue: '$100,000+ in 20 Days',
    roas: '5.6x ROAS',
    badge: 'Six Figures in 20 Days',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg'
  },
  {
    id: 'reel-8',
    shortcode: 'DbBMshACSuX',
    url: 'https://www.instagram.com/reel/DbBMshACSuX/',
    brand: 'DTC Fitness Brand',
    hook: 'Tension, Stress & Recovery Breakdown',
    revenue: '€123,000 in 20 Days',
    roas: '5.2x ROAS',
    badge: 'Meta Ads Framework',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg'
  },
  {
    id: 'reel-9',
    shortcode: 'DUROF3LCFVO',
    url: 'https://www.instagram.com/reel/DUROF3LCFVO/',
    brand: 'Growth Blueprint',
    hook: 'The Exact Checklist to Scale From $50k to $500k/mo with bulletproof media buying.',
    revenue: '$500,000 / mo Scale',
    roas: '4.7x ROAS',
    badge: 'Growth System',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg'
  },
  {
    id: 'reel-10',
    shortcode: 'DP74SV-jYX2',
    url: 'https://www.instagram.com/reel/DP74SV-jYX2/',
    brand: 'Creative Testing Matrix',
    hook: 'Why 90% of UGC Ads Fail (And How to Fix It with proven direct response).',
    revenue: '$850,000 Scaled',
    roas: '5.0x ROAS',
    badge: 'High-Converting UGC',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_4_DbCVqzFhiLU.jpg'
  },
  {
    id: 'reel-11',
    shortcode: 'DPI2h3TARq_',
    url: 'https://www.instagram.com/reel/DPI2h3TARq_/',
    brand: 'Media Buying Masterclass',
    hook: 'Budget Allocation Secrets for High Ad Spend & scaling bids without breaking CPA.',
    revenue: '$1,200,000 / mo',
    roas: '4.6x ROAS',
    badge: 'Capital Scaling',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_5_DcO79UbMIFx.jpg'
  },
  {
    id: 'reel-12',
    shortcode: 'DOkyKKYjFbs',
    url: 'https://www.instagram.com/reel/DOkyKKYjFbs/',
    brand: 'Conversion Rate Optimization',
    hook: 'Sub-1s Mobile Speed: The Unfair CVR Multiplier that doubles checkout completions.',
    revenue: '4.6% CVR Lift',
    roas: '5.4x ROAS',
    badge: 'Shopify Speed',
    category: 'CRO & Funnels',
    image: '/assets/instagram_thumbs/insta_6_DZcJNI0EhWY.jpg'
  },
  {
    id: 'reel-13',
    shortcode: 'DN6X3QjDZBD',
    url: 'https://www.instagram.com/reel/DN6X3QjDZBD/',
    brand: 'Brand Scaling Hacks',
    hook: 'From Product Validation to 8-Figure Exit: The step-by-step brand transition roadmap.',
    revenue: '$8,500,000 Valuation',
    roas: '4.9x ROAS',
    badge: 'Brand Architecture',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg'
  },
  {
    id: 'reel-14',
    shortcode: 'DN6WAbIkwOZ',
    url: 'https://www.instagram.com/reel/DN6WAbIkwOZ/',
    brand: 'Ad Account Teardown',
    hook: 'Audience Overlap & CPA Bottleneck Elimination across Advantage+ campaigns.',
    revenue: '-38% CPA Reduction',
    roas: '5.1x ROAS',
    badge: 'Account Audit',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg'
  },
  {
    id: 'reel-15',
    shortcode: 'DNnJohXymjy',
    url: 'https://www.instagram.com/reel/DNnJohXymjy/',
    brand: 'Direct Response Studio',
    hook: 'The 3-Second Hook Rule for Short-Form Video that drops customer acquisition cost in half.',
    revenue: '4.2M Views Generated',
    roas: '5.8x ROAS',
    badge: 'Viral Hook Matrix',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg'
  },
  {
    id: 'reel-16',
    shortcode: 'DNYVXIDtKg2',
    url: 'https://www.instagram.com/reel/DNYVXIDtKg2/',
    brand: 'Winner Scaling Engine',
    hook: 'How We Hit 6-Figures With a Single Winning Creative angle by aggressive horizontal testing.',
    revenue: '$140,000 in 14 Days',
    roas: '6.2x ROAS',
    badge: 'Winner Scaling',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_4_DbCVqzFhiLU.jpg'
  },
  {
    id: 'reel-17',
    shortcode: 'DL70gbmMBUK',
    url: 'https://www.instagram.com/reel/DL70gbmMBUK/',
    brand: 'Omnichannel Growth',
    hook: 'Scaling Meta Ads with Google Performance Max and high-intent keyword capture.',
    revenue: '$2,100,000 Omnichannel',
    roas: '4.75x ROAS',
    badge: 'Multi-Channel Scale',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_5_DcO79UbMIFx.jpg'
  },
  {
    id: 'reel-18',
    shortcode: 'DKnRV4FzvbQ',
    url: 'https://www.instagram.com/reel/DKnRV4FzvbQ/',
    brand: 'Customer Retention Engine',
    hook: 'Post-Purchase Upsells & LTV Maximization with zero customer churn.',
    revenue: '+44% LTV Surge',
    roas: '5.3x ROAS',
    badge: 'Retention Architecture',
    category: 'CRO & Funnels',
    image: '/assets/instagram_thumbs/insta_6_DZcJNI0EhWY.jpg'
  },
  {
    id: 'reel-19',
    shortcode: 'DJzgk9JSPyp',
    url: 'https://www.instagram.com/reel/DJzgk9JSPyp/',
    brand: 'Broad Targeting Engine',
    hook: 'Stop Hyper-Targeting: Broad Audience Mastery in 2026 for exponential scale.',
    revenue: '$620,000 / mo Scale',
    roas: '4.85x ROAS',
    badge: 'Broad Scale System',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg'
  },
  {
    id: 'reel-20',
    shortcode: 'DJoNUT3yJS_',
    url: 'https://www.instagram.com/reel/DJoNUT3yJS_/',
    brand: 'High Ticket DTC',
    hook: 'How to Sell $200+ Products on Cold Traffic with high-converting trust architecture.',
    revenue: '$1,100,000 / mo',
    roas: '4.5x ROAS',
    badge: 'High AOV DTC',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg'
  },
  {
    id: 'reel-21',
    shortcode: 'DJRz965uaDX',
    url: 'https://www.instagram.com/reel/DJRz965uaDX/',
    brand: 'Creative Ideation Studio',
    hook: '5 Creative Angles That Always Convert for eCommerce across competitive DTC niches.',
    revenue: '85+ Winning Angles',
    roas: '5.5x ROAS',
    badge: 'Angle Matrix',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg'
  },
  {
    id: 'reel-22',
    shortcode: 'DIOrrRMyBa_',
    url: 'https://www.instagram.com/reel/DIOrrRMyBa_/',
    brand: 'Ad Spend Scaling Rules',
    hook: 'When to Scale Up vs Kill Underperforming Ad Sets with exact mathematical criteria.',
    revenue: '$50M+ Ad Spend',
    roas: '4.9x ROAS',
    badge: 'Media Buying Rules',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_4_DbCVqzFhiLU.jpg'
  },
  {
    id: 'reel-23',
    shortcode: 'DH6I05IynDN',
    url: 'https://www.instagram.com/reel/DH6I05IynDN/',
    brand: 'DTC Eyewear & Fashion',
    hook: 'Cheaper Than Your Gym Membership 🌿 • Price-anchor shock testing for DTC apparel.',
    revenue: '$1,400,000 / Month',
    roas: '4.85x ROAS',
    badge: 'Verified Scale',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_5_DcO79UbMIFx.jpg'
  },
  {
    id: 'reel-24',
    shortcode: 'DHn8tadynaR',
    url: 'https://www.instagram.com/reel/DHn8tadynaR/',
    brand: 'Ecom Logistics & Margin',
    hook: 'Cash-Flow Optimization for Aggressive 8-Figure Scale without inventory bottlenecks.',
    revenue: '$3,200,000 Annual Run',
    roas: '5.1x ROAS',
    badge: 'Cash Flow Shield',
    category: 'Zero to Scale',
    image: '/assets/instagram_thumbs/insta_6_DZcJNI0EhWY.jpg'
  },
  {
    id: 'reel-25',
    shortcode: 'DHP7KQXS2Ki',
    url: 'https://www.instagram.com/reel/DHP7KQXS2Ki/',
    brand: 'Rapid Creative Testing',
    hook: 'Testing 45+ UGC Concepts Every Week on a fixed delivery cadence.',
    revenue: '12M+ Impressions',
    roas: '5.7x ROAS',
    badge: 'Creative Velocity',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg'
  },
  {
    id: 'reel-26',
    shortcode: 'DHPy8S_Mml_',
    url: 'https://www.instagram.com/reel/DHPy8S_Mml_/',
    brand: 'TikTok & Meta Funnels',
    hook: 'How to Bridge Social Attention to Shopify Checkouts with frictionless 1-page flows.',
    revenue: '$780,000 Scaled',
    roas: '4.6x ROAS',
    badge: 'Funnel Architecture',
    category: 'Meta Scaling',
    image: '/assets/instagram_thumbs/insta_2_DbBMshACSuX.jpg'
  },
  {
    id: 'reel-27',
    shortcode: 'DHOlSOcOczF',
    url: 'https://www.instagram.com/reel/DHOlSOcOczF/',
    brand: 'Brand Value & Moats',
    hook: 'Building a Brand Moat That Defends Against Competitors and commands premium pricing.',
    revenue: '8-Figure Brand Equity',
    roas: '5.0x ROAS',
    badge: 'Brand Authority',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_3_C9CPs88t1qa.jpg'
  },
  {
    id: 'reel-28',
    shortcode: 'DHG17Xnt_lV',
    url: 'https://www.instagram.com/reel/DHG17Xnt_lV/',
    brand: 'Psychological Triggers',
    hook: 'The 4 Psychological Triggers That Force Impulse Buys on cold traffic landing pages.',
    revenue: '+52% CVR Boost',
    roas: '5.4x ROAS',
    badge: 'Consumer Psychology',
    category: 'Creative Hooks',
    image: '/assets/instagram_thumbs/insta_4_DbCVqzFhiLU.jpg'
  },
  {
    id: 'reel-29',
    shortcode: 'DHBkLrTpy1W',
    url: 'https://www.instagram.com/reel/DHBkLrTpy1W/',
    brand: 'Holiday & Q4 Scale',
    hook: 'Black Friday / Q4 Scaling Architecture That Never Crashes under heavy media spend.',
    revenue: '$2,400,000 Q4 Record',
    roas: '6.5x ROAS',
    badge: 'Q4 Record Scaling',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_5_DcO79UbMIFx.jpg'
  },
  {
    id: 'reel-30',
    shortcode: 'DGgZKPbSjNa',
    url: 'https://www.instagram.com/reel/DGgZKPbSjNa/',
    brand: 'Friction Elimination CRO',
    hook: '3 Critical Friction Points Killing Your Mobile Checkout and how to fix them today.',
    revenue: '-28% Abandonment',
    roas: '5.2x ROAS',
    badge: 'Checkout Architecture',
    category: 'CRO & Funnels',
    image: '/assets/instagram_thumbs/insta_6_DZcJNI0EhWY.jpg'
  },
  {
    id: 'reel-31',
    shortcode: 'DGV7gMeNXYM',
    url: 'https://www.instagram.com/reel/DGV7gMeNXYM/',
    brand: 'Gaurav Kapoor Masterclass',
    hook: 'Real ad accounts. Real ad spend. Live revenue dashboards. Some people talk eCommerce, we engineer predictable scale.',
    revenue: '$50M+ Managed',
    roas: '4.85x Blended',
    badge: 'Verified Live Dashboards',
    category: '8-Figure Proof',
    image: '/assets/instagram_thumbs/insta_1_Db3hW_mupo1.jpg'
  }
];

export const instagramReelsData = liveResultsProof;

